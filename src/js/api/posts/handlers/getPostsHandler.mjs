...
export const getPostsHandler = async () => {
  ...
  try {
    ...
    // const { name: user, avatar } = getFromStorage("user");
    // const profileImage = avatar.url;

    posts.forEach((post) => {
      const postTitle = post?.title || "New Post";
      const postBody = post?.body || "";
      const postImage = post?.media?.url ? post.media.url : "";
      // If image doesn't exist, don't display alt. Else if image exists and alt text doesn't exist, use title as alt. If image and alt exist use alt text.
      let postAltText = "";
      if (!postImage) {
        postAltText = "";
      } else if (postImage && !post?.media.alt) {
        postAltText = post?.title || "";
      } else {
        postAltText = post.media.alt;
      }

      const postCreated = post?.created || "";
      const timeAgo = getTimeAgo(postCreated);
      displayPost(postImage, postAltText, postTitle, postBody, timeAgo, postTitle);
      console.log(post);
    });
  } catch (err) {
    console.error(err);
    displayMessage(messageContainer, "error", "Failed to display posts. Try again later..");
  }
};

const displayPost = (postImage, altText, postTitle, postBody, timeAgo) => {
...
const getTimeAgo = (created) => {
  ...
  const diff = Math.floor((currentDate - postCreated) / 1000);
